...
export class SlickComponent implements AfterViewInit, OnDestroy {
    ...
    initSlick() {
        ...
        this.zone.runOutsideAngular(() => {
            ...
            this.$instance.on('afterChange', (event: any, slick: any, currentSlide: any) => {
                self.zone.run(() => {
                    self.afterChange.emit({ event, slick, currentSlide });
                });
            });

            this.$instance.on('beforeChange', (event: any, slick: any, currentSlide: any, nextSlide: any) => {
                self.zone.run(() => {
                    self.beforeChange.emit({ event, slick, currentSlide, nextSlide });
                });
            });

            this.$instance.on('breakpoint', (event: any, slick: any, breakpoint: any) => {
                self.zone.run(() => {
                    self.breakpoint.emit({ event, slick, breakpoint });
                });
            });

            this.$instance.on('destroy', (event: any, slick: any) => {
                self.zone.run(() => {
    ...
    removeSlide(slickItem: SlickItemDirective) {
        ...
        this.slides = this.slides.filter((s:any) => s !== slickItem);
    }
